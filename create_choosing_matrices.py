...
def create_all_choosing_matrices(cache_dir='cached_relus/cifar100/resnet18'):
    with open(osp.join(cache_dir, 'activations.pkl'), 'rb') as f:
    ...
    with open(osp.join(cache_dir, '1K_prototypes_indices_p=0.pkl'), 'rb') as f:
        ...
        for sub_index in [0, 1]:
            for act_index in [1, 2]:
                ...
                os.makedirs(os.path.join(cache_dir,
                            ...
                            exist_ok=True)
                with open(osp.join(cache_dir, 'layer_name_to_choosing_matrix',
...
if __name__ == "__main__":
    args = parse_args()
    create_all_choosing_matrices(args.cache_dir)
